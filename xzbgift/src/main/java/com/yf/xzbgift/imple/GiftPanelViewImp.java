...
import com.yf.xzbgift.imple.adapter.GiftViewPagerAdapter;
...
public class GiftPanelViewImp extends BottomSheetDialog implements IGiftPanelView {
                        ...
                        if (is_lw) {
                            ...
                            if (giftInfo != null && mGiftPanelDelegate != null) {
                                if (giftInfo.gift_count.equals("0")) {
                                    Toast.makeText(mContext, "数量为0将使用钻石进行发送", Toast.LENGTH_SHORT).show();
                                    mGiftPanelDelegate.onGiftItemClick(giftInfo);
                                } else {
    ...
    public void notiBbGift() {
        ...
        if (giftInfo.gift_count.equals("0") && giftInfo.price > Integer.parseInt(money_zuan)) {
            mtv_cz.setText("充值");
            is_chongz = true;
            return;
        }
        ...
        if (is_lw) {
            ...
            mGiftController.setGiftClickListener(new GiftController.GiftClickListener() {
                ...
                public void onClick(int position, GiftInfo giftInfo) {
                    if (giftInfo.price > Integer.parseInt(money_zuan)) {
                        mtv_cz.setText("充值");
                        is_chongz = true;
                        return;
                    }
...
//                            initGiftData(giftInfoList);
    ...
    private void changeSelect() {
        if (is_lw) {
            ...
            if (mGiftController.getSelectGiftInfo() != null && mGiftController.getSelectGiftInfo().price > Integer.parseInt(money_zuan)) {
                mtv_cz.setText("充值");
                is_chongz = true;
                return;
            }
    ...
    public void setMoney_zuan(String money_zuan) {
        ...
        if (mtv_money != null) {
            ...
            if (mGiftController != null && is_lw && mGiftController.getSelectGiftInfo() != null) {
                if (mGiftController.getSelectGiftInfo().price > Integer.parseInt(money_zuan)) {
                    mtv_cz.setText("充值");
                    is_chongz = true;
                    return;
                }
                ...
                if (mGiftController_bb.getSelectGiftInfo().gift_count.equals("0") && mGiftController_bb.getSelectGiftInfo().price > Integer.parseInt(money_zuan)) {
                    mtv_cz.setText("充值");
                    is_chongz = true;
                    return;
                }
