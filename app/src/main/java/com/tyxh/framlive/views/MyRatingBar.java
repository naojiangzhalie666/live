...
public class MyRatingBar extends LinearLayout {
    ...
    private OnRatChangeListener mOnRatChangeListener;
    private boolean show_other = true;
    ...
    public void setOnRatChangeListener(OnRatChangeListener onRatChangeListener) {
        mOnRatChangeListener = onRatChangeListener;
    }

    public void setShow_other(boolean show_other) {
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                if(halfStarMesureWidth ==0){
                    ...
                    Log.d(TAG, "onGlobalLayout: "+"cmeasuredSpace="+measuredSpace);
                    for (int i=0;i<contains.size();i++){
                        starMesureCenterX[i]=(2*i+1)* halfStarMesureWidth +i*measuredSpace;  //计算每颗星星中心到父控件的左边的距离
                        Log.d(TAG, "onGlobalLayout: "+"centerX[i]="+starMesureCenterX[i]);
                    }
                }
            }
        });
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_UP:
                Log.d(TAG, "onTouchEvent: "+"maxDx="+maxDx+"::maxDy="+maxDy);
                if(maxDx>range||maxDy>range){  //为滑动事件

                }else{  //为点击事件
                    Log.d(TAG, "onTouchEvent: "+"handleClick");
                    handleClick(downX);
                }
                maxDx=0;
                maxDy=0;
    ...
    public void setRating(float rating){
        ...
        if(mOnRatChangeListener!=null)
            mOnRatChangeListener.onRatingChangListener(rating);
        ...
        for(int i=0;i<5;i++){
            ...
            layoutParams.height=mHeight;
            if(mRating==0){
                layoutParams.width=show_other?mWidth:0;
                stars.get(0).setBackgroundResource(R.color.star_gray);
                stars.get(1).setBackgroundResource(R.color.star_gray);
                stars.get(2).setBackgroundResource(R.color.star_gray);
                stars.get(3).setBackgroundResource(R.color.star_gray);
                stars.get(4).setBackgroundResource(R.color.star_gray);
            }else if(mRating<=1){
                if(i==0){
                    stars.get(0).setBackgroundResource(R.color.star_gray_se);
                    ...
                    stars.get(3).setBackgroundResource(R.color.star_gray);
                    stars.get(4).setBackgroundResource(R.color.star_gray);
                    layoutParams.width=show_other?mWidth:0; //第二、三、四、五颗星星为空
                }
            }else if(mRating<=2){
                if(i==0){
                    stars.get(0).setBackgroundResource(R.color.star_gray_se);
                    ...
                    stars.get(1).setBackgroundResource(R.color.star_gray_se);
                    layoutParams.width=(int)(mWidth*(mRating-1));//第二颗星星宽度按照小数部分决定
                }else{
                    stars.get(2).setBackgroundResource(R.color.star_gray);
                    stars.get(3).setBackgroundResource(R.color.star_gray);
                    stars.get(4).setBackgroundResource(R.color.star_gray);
                    layoutParams.width=show_other?mWidth:0; //第三、四、五颗星星为空
                }
            }else if(mRating<=3){
                if(i<2){
                    stars.get(0).setBackgroundResource(R.color.star_gray_se);
                    ...
                    stars.get(2).setBackgroundResource(R.color.star_gray_se);
                    layoutParams.width=(int)(mWidth*(mRating-2));//第三颗星星宽度按照小数部分决定
                }else{
                    stars.get(3).setBackgroundResource(R.color.star_gray);
                    stars.get(4).setBackgroundResource(R.color.star_gray);
                    layoutParams.width=show_other?mWidth:0;//第四、五颗星星为空
                }
            }else if(mRating<=4){
                if(i<3){
                    stars.get(0).setBackgroundResource(R.color.star_yellow);
                    stars.get(1).setBackgroundResource(R.color.star_yellow);
                    stars.get(2).setBackgroundResource(R.color.star_yellow);
                    layoutParams.width=mWidth;//第一、二、三颗星星填满
                }else if(i==3){
                    stars.get(3).setBackgroundResource(R.color.star_yellow);
                    layoutParams.width=(int)(mWidth*(mRating-3));//第四颗星星宽度按照小数部分决定
                }else{
                    stars.get(4).setBackgroundResource(R.color.star_gray);
                    layoutParams.width=show_other?mWidth:0;//第五颗星星为空
                }
            }else if(mRating<=5){
                if(i<4){
                    stars.get(0).setBackgroundResource(R.color.star_red);
                    stars.get(1).setBackgroundResource(R.color.star_red);
                    stars.get(2).setBackgroundResource(R.color.star_red);
                    stars.get(3).setBackgroundResource(R.color.star_red);
                    layoutParams.width=mWidth;//第一、二、三、四颗星星填满
                }else if(i==4){
                    stars.get(4).setBackgroundResource(R.color.star_red);
                    layoutParams.width=(int)(mWidth*(mRating-4));//第五颗星星宽度按照小数部分决定
                }else{
                    layoutParams.width=show_other?mWidth:0;  //这句是多余的
                }
            }else{
    ...
    public interface OnRatChangeListener{
        void onRatingChangListener(float rating);
    }
}
