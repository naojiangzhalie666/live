...
public class GaojuAdapter extends RecyclerView.Adapter<GaojuAdapter.ViewHolder> {
    private Context mContext;
    private List<PropBean.RetDataBean> mLists;
    private LayoutInflater mInflater;
    private OnItemClickListener mOnItemClickListener;

    public GaojuAdapter(Context context, List<PropBean.RetDataBean> stringList) {
        mContext = context;
        mLists = stringList;
    ...
    public void onBindViewHolder(ViewHolder vh, int position) {
        PropBean.RetDataBean bean = mLists.get(position);
        vh.mItemGiftsNum.setText(""+bean.getCount());
        vh.mItemGiftsTitle.setText(bean.getTitle());
        Glide.with(mContext).load(bean.getPropPicUrl()).error(R.drawable.live_defaultimg).placeholder(R.drawable.live_defaultimg).into(vh.mItemGiftsImg);
